...
(function () {
  ...
  function updateProgress() {
    ...
    if (MOCK_DB.user) {
      ...
      MOCK_DB.user.level = lvl;
      userLevel.textContent = `Nivel ${lvl}`;
    }
  }
  ...
  function buildScanOptions() {
    const wrap = $("#scanOptions");
    wrap.innerHTML = "";
    CARDS.forEach((c) => {
      const owned = state.collected.has(c.id);
      const btn = document.createElement("button");
      btn.className = `scan-option ${owned ? "done" : ""}`;
      btn.disabled = owned;
      btn.style.setProperty("--c-color", c.color);
      btn.innerHTML = `<span class="scan-option-dot"></span> Simular QR — ${c.name} ${owned ? "✓" : ""}`;
      btn.addEventListener("click", async () => {
        closeModal($("#scanModal"));
        await delay(300);
        await unlockCard(c.id);
      });
  ...
  $("#userChip").addEventListener("click", () => {
    if (!MOCK_DB.user) openModal($("#authModal"));
  });
  ...
  $("#loginForm").addEventListener("submit", async (e) => {
    e.preventDefault();
    const [email] = e.target.querySelectorAll("input");
    const user = await api.login(email.value || "usuario@focus.edu");
    onAuthed(user);
  });
  ...
  $("#registerForm").addEventListener("submit", async (e) => {
    ...
    const user = await api.register(name.value || "Usuario", email.value || "usuario@focus.edu");
    onAuthed(user);
  });
